...
public class Model {
    ...
    Model() {}
     ...
     /*Account checkExists(int id) {
		...
		{
			conn c1 = new conn();
			ResultSet rs = c1.stmt.executeQuery("select * from Account where id = '"+id+"';");
            Account details = null;
			while(rs.next()) {
				// accStatus acs =  rs.getString("acc");
				details = new Account(rs.getString("name"), rs.getString("password"),rs.getString("email"), rs.getString("acc"), rs.getInt("id"), rs.getInt("phone"),rs.getInt("reputation"));
                break;
			}
